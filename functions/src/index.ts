...
export function snakeCasify(x: object) {
    ...
    const newKeyValuePairs = keys.map(k => {
        ...
        if (
            ...
            throw new Error(
                `Object must only contain primitive values or arrays of primitive values, error for key: ${k} in object: ${JSON.stringify(
                    x
                )}`
            );
        }
...
function callGcp(
    host: string,
    path: string,
    payload: Location | Study | Survey | StudyAccess | User | any
): Promise<any> {
    const options = {
        method: 'POST',
        headers: {
...
async function saveUserToSqlApi(apiHost: string, userRecord: UserRecord) {
    ...
    const userFromGcp = await callGcp(apiHost, '/saveNewUser', user);
...
export const newlyAuthenticatedUser = functions.auth
    .user()
    .onCreate(user =>
        saveUserToSqlApi(functions.config().gcp.cloud_functions_url, user)
    );
...
export const newStudyCreated = functions.firestore
    ...
    .onCreate(
        async (
            ...
            if (true || decodedToken.studyCreator) {
                await saveStudyToSqlApi(
                    functions.config().gcp.cloud_functions_url,
                    newStudy
                );
...
export const studyUpdated = functions.firestore
    .document('/study/{studyId}')
    .onUpdate(async ({ after, before }, ctx) => {
        const newValue = after.data();
        const previousValue = before.data();
        const previousSurveyors = previousValue.surveyors
            ? previousValue.surveyors
            : [];
...
export const newLocationAdded = functions.firestore
    .document('/study/{studyId}/location/{locationId}')
    .onCreate(
        async (
            snapshot: FirebaseFirestore.DocumentSnapshot,
            ctx: functions.EventContext
            ...
            try {
                ...
                return await callGcp(
                    functions.config().gcp.cloud_functions_url,
                    '/saveNewLocation',
                    location
                );
            } catch (error) {
                console.error(`[location ${JSON.stringify(snapshot.data())}] ${error}`);
            }
        }
    );

export const newSurveyCreated = functions.firestore
    .document('/study/{studyId}/survey/{surveyId}')
    .onCreate(
        async (
            snapshot: FirebaseFirestore.DocumentSnapshot,
            ctx: functions.EventContext
...
async function sendDataPointSnapshotToGcp(
    snapshot: FirebaseFirestore.DocumentSnapshot
) {
    ...
    return await callGcp(
        functions.config().gcp.cloud_functions_url,
        '/saveDataPointToStudy',
        dataPoint
    );
}
export const addDataPointToSurvey = functions.firestore
    .document('/study/{studyId}/survey/{surveyId}/dataPoints/{dataPointId}')
    .onCreate(
        async (
            snapshot: FirebaseFirestore.DocumentSnapshot,
            ctx: functions.EventContext
...
export const updateDataPointForSurvey = functions.firestore
    .document('/study/{studyId}/survey/{surveyId}/dataPoints/{dataPointId}')
    .onUpdate(async ({ after, before }, ctx) => {
        return await sendDataPointSnapshotToGcp(after);
    });

export const deleteDataPointForSurvey = functions.firestore
    .document('/study/{studyId}/survey/{surveyId}/dataPoints/{dataPointId}')
